...
#train_data_dir = '/data/laser_detection/work_dirs/RED_cascade_rcnn_r50_fpn_20e_coco/all_crop_box_by_name_train_rotate_112/'
train_data_dir = '/data/laser_detection/work_dirs/RED_cascade_rcnn_r50_fpn_20e_coco/data_split/train_new_rotate/'
...
def extract(model_path):
    ...
    print('Model components loaded. Building model...')
    model = GP.build_global_attention_pooling_model_cascade_attention([modelvgg, modelvgg2], class_num)
    ...
    for i, file_name in enumerate(validation_generator.filenames):
        if i % 10 == 0:
            print(f"Processing image {i + 1}/{len(validation_generator.filenames)}: {file_name}")
        v = validation_generator[i]
        prob = model.predict(v[0])[3]
        output = prob.argmax(-1)[0]
        fname2cls[os.path.basename(file_name)] = (output, prob[:, output][0])
    ...
    print(f"Model evaluation result: {len(result)}, {result}")
